...
NodePtr rb_tree_next(NodePtr node) noexcept {
  ...
  while (!rb_tree_is_lchild(node)) node = node->parent;
  return node->parent;
}
...
class rb_tree {
 ...
 public:
  ...
  void insert_multi(InputIterator first, InputIterator last) {
    ...
    for (; n > 0; --n, ++first) insert_multi(end(), *first);
  }
  ...
  void insert_unique(InputIterator first, InputIterator last) {
    ...
    THROW_LENGTH_ERROR_IF(node_count_ > max_size() - n,
                          "rb_tree<T, Comp>'s size too big");
    for (; n > 0; --n, ++first) insert_unique(end(), *first);
  }
...
typename rb_tree<T, Compare>::iterator rb_tree<T, Compare>::upper_bound(
  ...
  while (x != nullptr) {
    if (key_comp_(
            key, value_traits::get_key(x->get_node_ptr()->value))) {  // key < x
      y = x, x = x->left;
    } else {
      x = x->right;
    }
